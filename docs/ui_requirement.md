...
1. **`/setup`**
   - 入力完了→サーバー保存成功→`/users`
2. **`/users`**
   - 「とりまランチ？」押下→相互成立→マッチングモーダル→OK後`/chat`
...
- 「最新更新日時順」で上に来るよう並べる
- 1ユーザー1マッチルール
- 相互Likeでマッチ成立→両者の「未マッチ」状態はマッチ状態（is_matched=true）に→両方のユーザーにマッチ成立モーダル→`/chat`へ
- Likeやマッチは当日分のみ有効
...
5. **「とりまランチ？」(like)ボタン**
   - ボタンクリック→ `likes` にINSERT → 相互Like確認 → 成立なら `matches` 作成→ モーダル表示
6. **マッチング成立モーダル**
...
- **マッチング成立自動確認**
  ...
  - 相手ユーザーからのLikeによってマッチが成立した場合も自動的にモーダルを表示
...
2. **マッチキャンセルボタン** (画面上右)
   ...
   - **キャンセルされたユーザー** → 「相手がキャンセルしました」ダイアログ→OK押下で `/users`へ
...
- 基本的にモーダル・ダイアログは内容と確認ボタン(OKなど)を表示
- 確認ボタン押下時のみモーダル・ダイアログ消滅または画面遷移
